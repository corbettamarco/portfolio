    ...
    <TabPanel my="1em" w="90vw" minH="30rem">
      ...
      >
        <VStack w={["100%", "100%", "50%", "50%", "50%"]}>
          ...
          <Text color={"portfolio.500"} fontSize={['1.2rem','1.2rem','1.2rem','1.5rem','1.5rem']}>{job.role}</Text>
          <HStack pt={'1rem'}>
            <CalendarIcon />
            ...
            <Center>
              <Wrap w="100%" pt="2rem">
                  {job.icons?.map((icon, index) => {
                    return (
                      <Box m='1rem' fontSize={["4rem","4rem","4rem","5rem","5rem"]} w={["4rem","4rem","4rem","5rem","5rem"]} key={index}>
                        {icon}
                      </Box>
                    );
                  })}
